...
def create_experiment(self, run_continuous = False):
    ...
    # 1 ms delay to avoid collision of the last edge assignment of digital channels as there is at most 8 channel changes at a given time stamp
    ...
    for edge_index in range(self.sequence_num_rows):
        ...
        if config.dds_channels_number > 0:
            ...
            if edge_index == 0: #adding a 1 ms delay after 8 ttl channels because otherwise it ignores the first analog channel
                file.write(indentation + "delay(1*ms)\n")
            ...
            #Assigning fastino card values
                ...
                for index, channel in enumerate(self.experiment.sequence[edge_index].analog):
                    if channel.changed == True:
                        ...
                        # if edge_index == 0 and index > 0: #adds a delay only for the default edge and in sace of 
                        if index > 0:
                ...
                if number_of_channels_changed > 1:
                    file.write(indentation + "delay(-%d00*ns)\n" %(number_of_channels_changed-1))
        ...
        if config.mirny_channels_number > 0: # commented out 25/02/28 bc run_experiment was not working after mirny 
            for index, channel in enumerate(self.experiment.sequence[edge_index].mirny):
                if channel.changed == True:
                    ...
                    #     file.write(indentation + "self.mirny" + str(mirny_num) + "_ch" + str(channel_num) + ".set_frequency(%s*MHz)\n"%str(float(channel.frequency.for_python) + 55.0))  #mirny has 55 MHz lower limit, hence + 55.0 for default (28/05/2025)
                    #     if channel.state.value == 1:
                    ...
                    # else:
                    if abs(float(channel.frequency.for_python)) <= 1e-5:
                        file.write(indentation + "self.mirny" + str(mirny_num) + "_ch" + str(channel_num) + ".set_frequency(55.0*MHz)\n") #mirny has 55 MHz lower limit, hence + 55.0 for default (28/05/2025)
                    else:
...
def create_go_to_edge(self, edge_num, to_default = False):
    ...
    if config.analog_channels_number > 0:
        ...
        # Assigning fastino card changes
            ...
            #Since we do not care about timing here we can add a redundant delay of 100 ns
            for index, channel in enumerate(self.experiment.sequence[edge].analog):
                file.write(indentation + "delay(100*ns)\n")    
