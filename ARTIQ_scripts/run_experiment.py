...
class run_experiment(EnvExperiment):
    def build(self):
        ...
        self.df = list(np.linspace(170.000000, 200.000000, 100))
    ...
    def run(self):
        ...
        for run_index in range(6):   # run loop including camera warm-up: warmup = 5, actual = 1, total = 6
            ...
            for step in range(100):
                ...
                self.zotino0.write_dac(7, 0.0)
                self.zotino0.load()
                #Edge number 32 name of edge: State prep
                delay((0.1000000000000085)*ms)
                self.urukul1_ch2.set_att((0.0)*dB) 
                self.urukul1_ch2.set(frequency = (365.319)*MHz, amplitude = (29.0)/100 , phase = (0.0)/360)
                self.urukul1_ch2.sw.on() 
                #Edge number 33 name of edge: State prep end
                delay((0.05433333333338908)*ms)
                self.urukul1_ch2.set_att((0.0)*dB) 
                self.urukul1_ch2.set(frequency = (365.319)*MHz, amplitude = (29.0)/100 , phase = (0.0)/360)
                self.urukul1_ch2.sw.off() 
                #Edge number 34 name of edge: Quant By ramp begin
                delay((0.9456666666666109)*ms)
                for i in range(1, (20+1)):   # ramp up loop 
                    self.zotino0.write_dac(2, 1.55+0.3*i)
                    self.zotino0.write_dac(3, -0.95)
                    self.zotino0.load()
                    delay((10.00000000000000/20)*ms)  # for ramp up: time devided by steps 
                #Edge number 35 name of edge: Quant By ramp end
                self.zotino0.write_dac(2, 9.0)
                self.zotino0.load()
                #Edge number 36 name of edge: Raman trigger
                delay((44.98000000000009)*ms)
                self.ttl10.on()
                #Edge number 37 name of edge: Raman 2 pulse begin
                delay((0.01999999999991076)*ms)
                self.ttl10.off()
                self.urukul2_ch2.set_att((26.0)*dB) 
                self.urukul2_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                self.urukul2_ch2.sw.on() 
                #Edge number 38 name of edge: Raman 2 pulse end
                delay((0.1000000000000938)*ms)
                self.urukul2_ch2.set_att((30.0)*dB) 
                self.urukul2_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                self.urukul2_ch2.sw.off() 
                #Edge number 39 name of edge: Push begin
                delay((1.0)*ms)
                self.zotino0.write_dac(7, 5.0)
                self.zotino0.load()
                #Edge number 40 name of edge: Push end
                delay((0.04999999999989768)*ms)
                self.zotino0.write_dac(7, 0.0)
                self.zotino0.load()
                #Edge number 41 name of edge: Storage end (free space det.)
                delay((28.2740000000001)*ms)
                self.zotino0.write_dac(8, 0.01)
                self.zotino0.write_dac(9, 0.01)
                self.zotino0.load()
                #Edge number 42 name of edge: Z cam trigger (meas)
                delay((1.0)*ms)
                if camera_enabled:
                    self.ttl8.on()
                else:
                    self.ttl8.off()
                #Edge number 43 name of edge: Exposure begin
                ...
                self.urukul1_ch0.set(frequency = (80.0)*MHz, amplitude = (40.0)/100 , phase = (0.0)/360)
                self.urukul1_ch0.sw.on() 
                #Edge number 44 name of edge: Exposure end
                ...
                self.urukul1_ch0.set(frequency = (80.0)*MHz, amplitude = (40.0)/100 , phase = (0.0)/360)
                self.urukul1_ch0.sw.off() 
                #Edge number 45 name of edge: Return dipole trap / Bias fields
                delay((0.1000000000000085)*ms)
                self.zotino0.write_dac(1, -5.0)
                self.zotino0.write_dac(2, 3.0)
                self.zotino0.write_dac(3, -5.0)
                self.zotino0.load()
                #Edge number 46 name of edge: Calculate fb
                delay((24.999999999999996)*ms)
                SSB = calculate_SSB(SSB,sampleSPOL)
                #Edge number 47 name of edge: Return dipole trap end
                delay((24.9999999999999)*ms)
