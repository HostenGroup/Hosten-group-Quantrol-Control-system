...
class run_experiment(EnvExperiment):
    def build(self):
        ...
        self.dt = list(np.linspace(0.000000, 0.020000, 101))
        ...
        for run_index in range(6):   # run loop including camera warm-up: warmup = 5, actual = 1, total = 6
            ...
            for run in range(3):
                for step1 in range(101):
                    ...
                    delay(((449.7550000000001+self.dt[step1]+1.0+0.05)-(449.7550000000001+self.dt[step1]+1.0))*ms)
                    self.zotino0.write_dac(7, 0.0)
                    self.zotino0.load()
                    #Edge number 43 name of edge: Storage end (free space det.)
                    delay(((449.7550000000001+self.dt[step1]+1.0+0.05+15.0)-(449.7550000000001+self.dt[step1]+1.0+0.05))*ms)
                    self.zotino0.write_dac(8, 0.01)
                    self.zotino0.write_dac(9, 0.01)
                    self.zotino0.load()
                    #Edge number 44 name of edge: Z cam trigger (meas)
                    delay(((449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0)-(449.7550000000001+self.dt[step1]+1.0+0.05+15.0))*ms)
                    if camera_enabled:
                        self.ttl8.on()
                    else:
                        self.ttl8.off()
                    #Edge number 45 name of edge: Exposure begin
                    delay(((449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03)-(449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0))*ms)
                    self.ttl8.off()
                    self.urukul0_ch2.set_att((6.5)*dB) 
                    self.urukul0_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                    self.urukul0_ch2.sw.on() 
                    ...
                    delay(((449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3)-(449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03))*ms)
                    self.urukul0_ch2.set_att((6.5)*dB) 
                    self.urukul0_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                    self.urukul0_ch2.sw.off() 
                    self.urukul1_ch0.set_att((10.0)*dB) 
                    ...
                    delay(((449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3+0.1)-(449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3))*ms)
                    self.zotino0.write_dac(8, 0.75)
                    self.zotino0.write_dac(9, 0.45)
                    self.zotino0.load()
                    self.urukul1_ch1.set_att((0.5)*dB) 
                    ...
                    delay(((449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3+0.1)-(449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3+0.1))*ms)
                    for i in range(1, (20+1)):   # ramp up loop 
                        self.zotino0.write_dac(2, 9-((9-2.03)/20)*i)
                        self.zotino0.write_dac(3, -0.95-((6-0.95)/20)*i)
                        self.zotino0.load()
                        delay((((449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3+0.1+20.0)-(449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3+0.1))/20)*ms)  # for ramp up: time devided by steps 
                    #Edge number 49 name of edge: By3/Bz3 ramp end
                    self.zotino0.write_dac(2, 2.031)
                    self.zotino0.write_dac(3, -6.0)
                    self.zotino0.load()
                    #Edge number 50 name of edge: Calculate feedback ssb
                    delay(((449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3+0.1+20.0+40.0)-(449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3+0.1+20.0))*ms)
                    SSB = calculate_SSB(SSB,sampleSPOL)
                    #Edge number 51 name of edge: Final edge
                    delay(((449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3+0.1+20.0+40.0+5.0)-(449.7550000000001+self.dt[step1]+1.0+0.05+15.0+1.0+0.03+0.3+0.1+20.0+40.0))*ms)
