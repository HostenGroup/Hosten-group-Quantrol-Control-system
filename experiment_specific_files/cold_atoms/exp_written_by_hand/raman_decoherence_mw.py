...
class run_experiment(EnvExperiment):
    def build(self):
        ...
        self.dphase = list(np.linspace(0.000000, 360.000000, 101))
    ...
    def run(self):
        ...
        for run_index in range(6):   # run loop including camera warm-up: warmup = 5, actual = 1, total = 6
            ...
            for step1 in range(101):
                ...
                self.urukul1_ch2.set(frequency = (365.319425)*MHz, amplitude = (29.0)/100 , phase = (0.0)/360,profile = 0)
                self.urukul1_ch2.set(frequency = (365.3)*MHz, amplitude = (29.0)/100 , phase = (0.0)/360, profile = 1)
                self.urukul1_ch2.set(frequency = (365.3)*MHz, amplitude = (29.0)/100 , phase = (dphase)/360, profile =3)
                ...
                delay((0.0067500000000028045)*ms)
                self.urukul2_ch2.sw.off()
                #Edge number 41 name of edge: Delay
                t_pulse2 = now_mu() + self.core.seconds_to_mu(0.5*us)
                self.urukul1_cpld.set_profile(3)
                at_mu(t_pulse2)
                #Edge number 42 name of edge: Raman Pulse 2 begin
                self.urukul2_ch2.sw.on()
                #Edge number 43 name of edge: Raman Pulse 2 end
                delay((0.0067500000000029821)*ms)
                self.urukul2_ch2.sw.off()
                self.urukul2_cpld.set_profile(0)
                self.urukul1_cpld.set_profile(0)
                #Edge number 44 name of edge: Push begin
                delay((1.0)*ms)
                self.zotino0.write_dac(7, 5.0)
                self.zotino0.load()
                self.urukul1_ch2.set_att((0.0)*dB) 
                ...
                self.urukul1_ch2.sw.off() 
                #Edge number 45 name of edge: Push end
                delay((0.050000000000010036)*ms)
                self.zotino0.write_dac(7, 0.0)
                self.zotino0.load()
                #Edge number 46 name of edge: Storage end (free space det.)
                delay((15.0)*ms)
                self.zotino0.write_dac(8, 0.01)
                self.zotino0.write_dac(9, 0.01)
                self.zotino0.load()
                #Edge number 47 name of edge: Z cam trigger (meas)
                delay((1.0)*ms)
                if camera_enabled:
                    self.ttl8.on()
                else:
                    self.ttl8.off()
                #Edge number 48 name of edge: Exposure begin
                delay((0.02999999999995895)*ms)
                self.ttl8.off()
                self.urukul0_ch2.set_att((6.5)*dB) 
                self.urukul0_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                self.urukul0_ch2.sw.on() 
                ...
                self.urukul1_ch0.set(frequency = (80.0)*MHz, amplitude = (40.0)/100 , phase = (0.0)/360)
                self.urukul1_ch0.sw.on() 
                #Edge number 49 name of edge: Exposure end
                delay((0.3000000000000043)*ms)
                self.urukul0_ch2.set_att((6.5)*dB) 
                self.urukul0_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                self.urukul0_ch2.sw.off() 
                self.urukul1_ch0.set_att((10.0)*dB) 
                self.urukul1_ch0.set(frequency = (80.0)*MHz, amplitude = (40.0)/100 , phase = (0.0)/360)
                self.urukul1_ch0.sw.off() 
                #Edge number 50 name of edge: Return dipole trap
                delay((0.10000000000004672)*ms)
                self.zotino0.write_dac(8, 0.75)
                self.zotino0.write_dac(9, 0.45)
                self.zotino0.load()
                self.urukul1_ch1.set_att((0.5)*dB) 
                self.urukul1_ch1.set(frequency = (182.65)*MHz, amplitude = (30.0)/100 , phase = (0.0)/360)
                self.urukul1_ch1.sw.on() 
                #Edge number 51 name of edge: By3/Bz3 ramp begin
                for i in range(1, (20+1)):   # ramp up loop 
                    self.zotino0.write_dac(2, 9-((9-2.03)/20)*i)
                    self.zotino0.write_dac(3, -0.95-((6-0.95)/20)*i)
                    self.zotino0.load()
                    delay((20.000000000000000/20)*ms)  # for ramp up: time devided by steps 
                #Edge number 52 name of edge: By3/Bz3 ramp end
                self.zotino0.write_dac(2, 2.031)
                self.zotino0.write_dac(3, -6.0)
                self.zotino0.load()
                #Edge number 53 name of edge: Calculate feedback ssb
                delay((39.99999999999996)*ms)
                SSB = calculate_SSB(SSB,sampleSPOL)
                #Edge number 54 name of edge: Final edge
                delay((5.0)*ms)
