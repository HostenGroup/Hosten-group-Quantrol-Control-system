    ...
    def run(self):
        ...
        for run_index in range(6):   # run loop including camera warm-up: warmup = 5, actual = 1, total = 6
            ...
            for step1 in range(101):
                ...
                self.urukul1_ch3.set(frequency = (228.601)*MHz, amplitude = (10.0)/100 , phase = (0.0)/360)
                ...
                delay((0.006800000000000284)*ms)
                self.urukul2_ch2.sw.off() 
                #delay
                delay(1.5*us)
                delay(-1.1*us)
                self.urukul1_ch2.set(frequency = (365.3)*MHz, amplitude = (29.0)/100 , phase = (self.dphase[step1])/360)
                #Edge number 42 name of edge: Raman Pulse 2 begin
                self.urukul2_ch2.sw.on() 
                #Edge number 43 name of edge: Raman Pulse 2 end
                delay((0.006800000000000284)*ms)
                self.urukul2_ch2.sw.off() 
                #Edge number 44 name of edge: Storage end (free space det.)
                delay((0.9924999999999997)*ms)
                self.zotino0.write_dac(8, 0.01)
                self.zotino0.write_dac(9, 0.01)
                self.zotino0.load()
                #Edge number 45 name of edge: Push begin
                delay((0.5)*ms)
                self.zotino0.write_dac(7, 5.0)
                self.zotino0.load()
                delay(-1.3*us)
                ...
                self.urukul1_ch2.set(frequency = (365.3)*MHz, amplitude = (29.0)/100 , phase = (0.0)/360)
                self.urukul1_ch2.sw.off() 
                #Edge number 46 name of edge: Push end
                delay((0.05000000000000426)*ms)
                self.zotino0.write_dac(7, 0.0)
                self.zotino0.load()
                #Edge number 47 name of edge: Z cam trigger (meas)
                delay((0.6000000000000485)*ms)
                if camera_enabled:
                    self.ttl8.on()
                else:
                    self.ttl8.off()
                #Edge number 48 name of edge: Exposure begin
                delay((0.029999999999946958)*ms)
                self.ttl8.off()
                delay(-1.3*us)
                self.urukul0_ch2.set_att((6.5)*dB) 
                delay(-1.1*us)
