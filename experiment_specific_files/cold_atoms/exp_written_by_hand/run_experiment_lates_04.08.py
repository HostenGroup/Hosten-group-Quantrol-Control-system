...
class run_experiment(EnvExperiment):
    def build(self):
        ...
        self.df = list(np.linspace(365.200000, 365.400000, 901))
        #self.dt = list(np.linspace(0, 0.04, 51))
        #self.dphase = list(np.linspace(0, 360, 181))
    ...
    def run(self):
        ...
        for run_index in range(6):   # run loop including camera warm-up: warmup = 5, actual = 1, total = 6
            ...
            for step1 in range(901):
                ...
                self.urukul2_ch0.set(frequency = (df)*MHz, amplitude = (29.0)/100 , phase = (0.0)/360,profile=1)
                self.urukul2_ch0.set(frequency = (df)*MHz, amplitude = (29.0)/100 , phase = (dphase)/360,profile=3)
                ...
                #Delay between pulse
                t_delay = now_mu() + self.core.seconds_to_mu(100*us)
