...
def create_experiment(self, run_continuous = False, multiple_runs = False):
    ...
    file.write(indentation + "from main.run_experiment_utils import * \n")
    ...
    if not run_continuous:
        ...
        if stop_at_end_of_sequence_flag == True:
            ...
            # indentation = indentation[:-4]
            file.write(indentation + "delete_run_active_flag(self)\n")
    ...
    if stop_at_end_of_sequence_flag == True:
        ...
        indentation += '    '
        file.write(indentation + "create_run_active_flag() \n")
        # file.write(indentation + "self.repo_path = Path(__file__).resolve().parent.parent\n")
        # file.write(indentation + "path = Path(self.repo_path) / 'ARTIQ_scripts' / 'run_active_flag.txt' \n")
    ...
    if stop_at_end_of_sequence_flag == True:
        ...
        # indentation = indentation[:-4]        
        file.write(indentation + "send_int_file_to_artiq() \n")
