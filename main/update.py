...
def sequence_tab(self):
    ...
    while something_changed and iterations < iterations_limit:
        ...
        for row, edge in enumerate(self.experiment.sequence):
            ...
            try:
                (edge.expression, edge.evaluation, edge.for_python, edge.is_scanned, edge.is_ramped, is_sampled, is_derived, is_lookup) = self.decode_input(expression)
                ...
                if edge.id in self.experiment.variables: # in case of deleting an edge there is no self.experiment.variables[edge.id] since we delete it in oder to check whether it has been used anywhere or not
                    if self.experiment.variables[edge.id].is_scanned != edge.is_scanned or self.experiment.variables[edge.id].for_python != edge.for_python or self.experiment.variables[edge.id].is_ramped != edge.is_ramped:
                        something_changed = True
                        self.experiment.variables[edge.id].is_scanned = edge.is_scanned
                        self.experiment.variables[edge.id].is_ramped = edge.is_ramped
                    ...
                    if update_expressions_and_evaluations:
                        ...
                        try:
                            if setting == 0: #frequency
                                ...
                                channel_entry.expression = str(round(float(channel_entry.expression))) # Keep only up to 3rd digit (0.1234 --> 0.123) of phase that is represented as 1 -- > 360. 0.001 --> 0.36 in degrees
...
def ramp_table(self):
    ...
    for row, variable in enumerate(self.experiment.ramped_variables):
        ...
        self.ramp_table_parameters.setItem(row,3, QTableWidgetItem(str(variable.functionramp)))
        self.ramp_table_parameters.setItem(row,4, QTableWidgetItem(str(variable.stepsramp)))
    self.update_on()
...
def variable_tables(self):
    ...
    if config.mirny_channels_number > 0:
        self.variables_table_row_count = len(self.experiment.new_variables)
        self.variables_table_mirny.setRowCount(self.variables_table_row_count)
        for row, variable in enumerate(self.experiment.new_variables):
            self.variables_table_mirny.setItem(row, 0, QTableWidgetItem(variable.name))
            if self.experiment.do_scan and variable.is_scanned: #Highlighting that the variable is used in a scan
                item = QTableWidgetItem("scanned")
                item.setFlags(Qt.NoItemFlags)
                self.variables_table_mirny.setItem(row, 1, item) 
            elif self.experiment.do_ramp and variable.is_ramped: #Highlighting that the variable is used in a ramp
                item = QTableWidgetItem("ramped") 
                item.setFlags(Qt.NoItemFlags) 
                self.variables_table_mirny.setItem(row, 1, item)
                ...
                self.variables_table_mirny.setItem(row, 1, QTableWidgetItem(str(variable.value)))  

    # UPDATING SLOW DDS TAB
    if config.slow_dds_channels_number > 0:
        self.variables_table_row_count = len(self.experiment.new_variables)
        self.variables_table_slow_dds.setRowCount(self.variables_table_row_count)
        for row, variable in enumerate(self.experiment.new_variables):
            self.variables_table_slow_dds.setItem(row, 0, QTableWidgetItem(variable.name))
            if self.experiment.do_scan and variable.is_scanned: #Highlighting that the variable is used in a scan
                item = QTableWidgetItem("scanned")
                item.setFlags(Qt.NoItemFlags)
                self.variables_table_slow_dds.setItem(row, 1, item) 
            elif self.experiment.do_ramp and variable.is_ramped: #Highlighting that the variable is used in a ramp
                item = QTableWidgetItem("ramped") 
                item.setFlags(Qt.NoItemFlags) 
                self.variables_table_slow_dds.setItem(row, 1, item)
                ...
                self.variables_table_slow_dds.setItem(row, 1, QTableWidgetItem(str(variable.value)))  

    # UPDATING SAMPLER TAB
