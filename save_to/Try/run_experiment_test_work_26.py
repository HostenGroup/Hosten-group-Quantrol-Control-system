...
class run_experiment(EnvExperiment):
    def build(self):
        ...
        self.datt = list(np.linspace(0.000000, 31.500000, 100))
        ...
        for run_index in range(4):   # run loop including camera warm-up: warmup = {warmup_runs}, actual = {actual_runs}, total = {total_runs}
            ...
            if run_index == 3:
                for _ in range(10):
                    ...
                    delay(200*ms)
            camera_enabled = (run_index >= 3)   # warm-up run check
            delay(100*ns)
            ...
            for step in range(100):
                ...
                self.urukul1_ch1.set(frequency = (179.95)*MHz, amplitude = (30.0)/100 , phase = (0.0)/360)
                ...
                self.urukul2_ch2.set_att((15.0)*dB) 
                ...
                self.zotino0.load()
                self.urukul0_ch2.set_att((31.5)*dB) 
                self.urukul0_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                self.urukul0_ch2.sw.off() 
                self.urukul0_ch3.set_att((19.0)*dB) 
                self.urukul0_ch3.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                ...
                delay((25.0)*ms)
                self.urukul0_ch2.set_att((29.0)*dB) 
                self.urukul0_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                self.urukul0_ch2.sw.on() 
                self.urukul1_ch0.set_att((28.0)*dB) 
                self.urukul1_ch0.set(frequency = (80.0)*MHz, amplitude = (40.0)/100 , phase = (0.0)/360)
                self.urukul1_ch0.sw.on() 
                ...
                self.urukul0_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                self.urukul0_ch2.sw.off() 
                self.urukul1_ch0.set_att((31.5)*dB) 
                self.urukul1_ch0.set(frequency = (80.0)*MHz, amplitude = (40.0)/100 , phase = (0.0)/360)
                self.urukul1_ch0.sw.off() 
                ...
                self.zotino0.write_dac(2, 9.0)
                self.zotino0.load()
                #Edge number 34 name of edge: Raman 2 pulse begin
                delay((25.0)*ms)
                self.urukul1_ch2.set_att((0.0)*dB) 
                self.urukul1_ch2.set(frequency = (365.319)*MHz, amplitude = (29.0)/100 , phase = (0.0)/360)
                self.urukul1_ch2.sw.on() 
                self.urukul2_ch2.set_att((self.datt[step])*dB) 
                self.urukul2_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                self.urukul2_ch2.sw.on() 
                #Edge number 35 name of edge: Raman 2 pulse end
                delay((0.05000000000001137)*ms)
                self.urukul1_ch2.set_att((0.0)*dB) 
                self.urukul1_ch2.set(frequency = (365.319)*MHz, amplitude = (29.0)/100 , phase = (0.0)/360)
                self.urukul1_ch2.sw.off() 
                self.urukul2_ch2.set_att((self.datt[step])*dB) 
                self.urukul2_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                self.urukul2_ch2.sw.off() 
                #Edge number 36 name of edge: Push begin
                delay((1.0)*ms)
                self.zotino0.write_dac(7, 0.0)
                self.zotino0.load()
                self.urukul2_ch2.set_att((15.0)*dB) 
                self.urukul2_ch2.set(frequency = (80.0)*MHz, amplitude = (20.0)/100 , phase = (0.0)/360)
                self.urukul2_ch2.sw.off() 
                #Edge number 37 name of edge: Push end
                delay((0.04999999999989768)*ms)
                self.zotino0.write_dac(7, 0.0)
                self.zotino0.load()
                #Edge number 38 name of edge: Storage end (free space det.)
                delay((23.5500000000002)*ms)
                self.zotino0.write_dac(8, 0.01)
                self.zotino0.write_dac(9, 0.01)
                self.zotino0.load()
                #Edge number 39 name of edge: Z cam trigger (meas)
                delay((1.0)*ms)
                if camera_enabled:
                    self.ttl8.on()
                else:
                    self.ttl8.off()
                #Edge number 40 name of edge: Exposure begin
                ...
                self.urukul1_ch0.set(frequency = (80.0)*MHz, amplitude = (40.0)/100 , phase = (0.0)/360)
                self.urukul1_ch0.sw.on() 
                #Edge number 41 name of edge: Exposure end
                ...
                self.urukul1_ch0.set(frequency = (80.0)*MHz, amplitude = (40.0)/100 , phase = (0.0)/360)
                self.urukul1_ch0.sw.off() 
                #Edge number 42 name of edge: Return dipole trap / Bias fields
                delay((0.1000000000000085)*ms)
                self.zotino0.write_dac(1, -6.0)
                self.zotino0.write_dac(2, 3.0)
                self.zotino0.write_dac(3, -6.0)
                self.zotino0.load()
                #Edge number 43 name of edge: Calculate fb
                delay((25.0)*ms)
                SSB = calculate_SSB(SSB,sampleSPOL)
                #Edge number 44 name of edge: Return dipole trap end
